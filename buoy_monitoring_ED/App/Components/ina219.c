...
static Energy_Status reg_write(uint8_t addr, uint8_t reg, uint16_t value) {
    uint8_t buf[3];
    buf[0] = reg;
...
static Energy_Status reg_read(uint8_t addr, uint8_t reg, uint16_t *value) {
    uint8_t buf[2];
    for(uint8_t i = 0; i < INA219_COMMS_REINTENTOS; i++) {
...
static Energy_Status INA219_getVoltage(INA219_Channel ch, int16_t *voltage_mV) {
    uint16_t raw;
...
static Energy_Status INA219_getCurrent(INA219_Channel ch, int16_t *current_mA) {
    uint16_t raw;
...
Energy_Status INA219_initAll(void) {
    for (int i = 0; i < INA219_COUNT; i++) {
        ...
        if(reg_write(s_i2c_addrs[i], REG_CONFIGURATION, CFG_CONFIGURATION) == ENERGY_ERR_COMMS) {
        	return ENERGY_ERR_COMMS;
        }
    }
    return ENERGY_OK;
...
Energy_Status INA219_getVoltageAndCurrent(INA219_Channel ch, Energy_Data *data) {
	if(INA219_getVoltage(ch, &(data->voltage_mV)) == ENERGY_OK) {
		if(INA219_getCurrent(ch, &(data->current_mA)) == ENERGY_OK) {
			return ENERGY_OK;
		}
	}
	return ENERGY_ERR_COMMS;
